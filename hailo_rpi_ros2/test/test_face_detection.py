...
from hailo_rpi_ros2.face_detection import FaceDetection
...
def face_detection_instance():
    ...
    with patch(
        ...
        patch(
        'hailo_apps_infra.face_detection_pipeline.'
        'GStreamerFaceDetectionApp'
    ) as mock_gstreamer_app, \
        patch(
        'hailo_apps_infra.gstreamer_app.'
        'GStreamerApp.create_pipeline'
    ) as mock_base_create_pipeline, \
        patch(
        'hailo_apps_infra.gstreamer_app.'
        'GStreamerApp.run'
    ) as mock_base_run:
        ...
        face_detection = FaceDetection(mock_frame_callback)
...
def test_app_callback(
        mock_get_caps, mock_get_numpy, mock_cvt_color, mock_put_text, mock_get_roi,
        mock_gallery, face_detection_instance):
